...
## 4. The Workflow (as-built per ADR-004)

1.  **Trigger:** A "Digital Exhaust" scan detects a potential breach.
2.  **Creation:** System creates a `FuryAuditTask`.
3.  **Assignment:** The Router assigns **3 auditors** anonymously (no social/historical link to committer).
4.  **Verification:** Each auditor reviews the artifact and submits a "Breach Confirmed" or "False Alarm" verdict.
5.  **Consensus:** 2-of-3 or 3-of-3 agreement → verdict accepted. 3-way split → escalation to senior auditor pool. There is no $1000 threshold.
...
| FR-5 | 2-of-3 or 3-of-3 consensus enforced for all contracts (per ADR-004; no $1000 threshold) | Integration test: submit breach; verify 3 auditors assigned and 2-of-3 required             |
...
- 2-of-3 or 3-of-3 consensus operational for all contracts (per ADR-004).
...
_Generated per METADOC Stage IV (Backcasting) & Gold Path SOP | Styx Architecture | v2.0.0_
